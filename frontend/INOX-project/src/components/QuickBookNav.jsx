...
const QuickBookNav = () => {
  ...
  const handleMovieData = async () => {
    try {
      ...
      setFetchedMovieData(movies);
    } catch (err) {
      console.error("Movie data not found", err);
      setFetchedMovieData([]);
    }
  };
  ...
  const handleCinemaData = async () => {
    try {
      const response = await axios.get("http://localhost:8080/open/cinemas/alls");
      const cinemas = Array.isArray(response.data) ? response.data : [];
      setFetchedCinemaData(cinemas);
    } catch (err) {
      console.error("Cinema data not found", err);
      setFetchedCinemaData([]);
    }
  };
