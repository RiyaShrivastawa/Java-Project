  ...
  const handleDelete = async () => {
    ...
    try {
      const response = await axios.delete(
        `http://localhost:8080/${deleteData?.comp}/delete/${deleteData?.data?.id}`,
        {
          headers: {
  ...
  return (
    isModalOpen && (
      <div className="modal-backdrop-deletenotify" onClick={handleCancel}>
        <div
          className="modal-container-deletenotify"
        ...
        >
          ...
          <div className="modal-actions-deletenotify">
            <button
              className="btn-deletenotify cancel-btn-deletenotify"
              onClick={handleCancel}
            >
              Cancel
            </button>
            <button
              className="btn-deletenotify delete-btn-deletenotify"
              onClick={handleDelete}
            >
              Delete
            </button>{" "}
            {/* Put api to delete the selected item */}
          </div>
        </div>
      </div>
    )
